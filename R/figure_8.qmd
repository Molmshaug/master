...
## Correlation between Hb removed and vo2max

#Hb correlation
blood_removal.hb <- blood.rm.data %>%
  select(id, test, hbmass) %>%
...
regression_table <- merge(blood_removal.hb, final_dataset, by = "id")
...
## Correlation between Hb removed and PO 15min
...
per_postphle <- performance.data %>%
 ...
 final_per <- merge(per_dataset, blood_removal.hb, by = "id")
